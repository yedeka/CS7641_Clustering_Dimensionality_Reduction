...
def pcakMeans(data, pcVal, label, metric):
    ...
    estimate_k({'features': pca_transformed_data}, label, 'plots/dr_clustering/kmeans/', metric)
    validate_k({'features': pca_transformed_data, 'labels': data['labels']}, 'plots/dr_clustering/kmeans/', label)


def pcaem(data, pcVal, label, metric):
    pca_transformed_data = pca.return_principal_components(data, pcVal)
    print('PCA transformed data shape  for em ', pca_transformed_data.shape)
    estimate_em_k({'features': pca_transformed_data}, label, 'plots/dr_clustering/em/', metric)
    validate_k({'features': pca_transformed_data, 'labels': data['labels']}, 'plots/dr_clustering/em/', label)
